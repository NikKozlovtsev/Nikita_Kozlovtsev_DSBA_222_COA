...
def consume_events(ch_client):
    ...
    и вставка в ClickHouse с корректным преобразованием timestamp.
    """
    ...
    for msg in consumer:
        ...
        # Если в конце есть 'Z', заменяем на '+00:00'
        ...
        ch_client.execute(
            "INSERT INTO default.events (user_id, entity_id, event, timestamp) VALUES",
            [(data['clientId'], data['postId'], ev_name, ev_ts)]
        )
    ...
    for msg in consumer:
        ...
        # Преобразуем ISO-строку в объект datetime с таймзоной UTC
        ts_str = data['timestamp']
        if ts_str.endswith('Z'):
            # "2025-05-30T12:20:53.542Z" → "2025-05-30T12:20:53.542+00:00"
            ts_str = ts_str[:-1] + '+00:00'
        ev_ts = datetime.fromisoformat(ts_str)

        # Вставляем уже datetime-объект в ClickHouse
        ch_client.execute(
            """
...
def serve():
    ...
    ch = Client(host=CLICKHOUSE_HOST, port=9000, database='default')
